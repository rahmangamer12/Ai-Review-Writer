  ...
  return (
    <div className="w-full">
      <div className="w-full max-w-4xl mx-auto">
        ...
        <AnimatePresence>
          {uploadedFiles.length > 0 && (
            <motion.div
              ...
              className="flex flex-wrap gap-1.5 sm:gap-2 mb-2 sm:mb-3"
            >
              {uploadedFiles.map(file => (
                <motion.div
                  ...
                  className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-2.5 py-1.5 sm:py-2 bg-white/[0.05] rounded-lg sm:rounded-xl border border-white/10 group hover:border-white/20 transition-colors"
                >
                  {file.preview ? (
                    <img
                      src={file.preview}
                  ...
                  <div className="flex flex-col min-w-0">
                    <span className="text-[10px] sm:text-xs font-medium truncate max-w-[60px] xs:max-w-[80px] sm:max-w-[120px]">{file.name}</span>
                    <span className="text-[8px] sm:text-[10px] text-white/40">{formatFileSize(file.size)}</span>
                  </div>
                  <button
                    onClick={() => removeFile(file.id)}
                    className="p-0.5 sm:p-1 rounded hover:bg-white/10 text-white/40 hover:text-white transition-colors"
                  >
        ...
        <div className={`relative transition-all duration-200 ${
        ...
        } border rounded-xl sm:rounded-2xl w-full`}>
          <div className="flex items-end gap-1 sm:gap-1.5 lg:gap-2 p-1.5 sm:p-2">
            ...
            <textarea
              ...
              className="flex-1 bg-transparent resize-none py-2 sm:py-2.5 lg:py-3 min-h-[36px] sm:min-h-[40px] max-h-[120px] sm:max-h-[150px] lg:max-h-[200px] text-sm sm:text-[15px] placeholder:text-white/30 focus:outline-none w-full"
              rows={1}
              disabled={disabled}
            />
            ...
            >
              {isVoiceActive ? <StopCircle className="w-4 h-4 sm:w-4.5 sm:h-4.5 lg:w-5 lg:h-5" /> : <Mic className="w-4 h-4 sm:w-4.5 sm:h-4.5 lg:w-5 lg:h-5" />}
            </button>
            ...
            >
              {isLoading ? (
                <Loader2 className="w-4 h-4 sm:w-4.5 sm:h-4.5 lg:w-5 lg:h-5 animate-spin" />
              ) : (
          ...
          <div className="flex items-center justify-between px-2 sm:px-3 py-1.5 sm:py-2 border-t border-white/[0.05]">
            <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-3 text-[9px] sm:text-[10px] text-white/30 min-w-0">
              <button
                onClick={onOpenModelSelector}
                className="flex items-center gap-1 hover:text-white transition-colors min-w-0"
              >
                <Wand2 className="w-2.5 h-2.5 sm:w-3 sm:h-3 shrink-0" />
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              <span className="text-[9px] sm:text-[10px] text-violet-400 font-medium whitespace-nowrap">AI Powered</span>
            </div>
          </div>
        </div>
      </div>
